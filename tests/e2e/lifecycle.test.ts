...
describe('Server Lifecycle', () => {
  ...
  describe('startup', () => {
    ...
    it('starts with custom environment variables', async () => {
      await client.start({
        AIRTABLE_ACCESS_TOKEN: 'custom-test-key',
  ...
  describe('tools listing', () => {
    ...
    it('includes expected Airtable tools', async () => {
      await client.start();
      const tools = await client.listTools();
      const toolNames = tools.map(t => t.name);
      expect(toolNames).toContain('airtable_list_bases');
      expect(toolNames).toContain('airtable_list_records');
      expect(toolNames).toContain('airtable_create_records');
    });
  });
});
