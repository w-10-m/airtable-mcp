/**
 * Logger - Centralized logging service for airtable-mcp
...
export class Logger {
  ...
  private log(level: LogLevel, action: string, message: string, metadata?: any): void {
    ...
    const sessionId = this.batcher?.getSessionId() || `airtable-mcp-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
