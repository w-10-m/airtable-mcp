...
describe('Config', () => {
  ...
  describe('loadConfig', () => {
    ...
    it('should load airtable configuration', () => {
      process.env.AIRTABLE_ACCESS_TOKEN = 'airtable_access_token';

      const config = loadConfig();

      expect(config.airtable.aIRTABLEACCESSTOKEN).toBe('airtable_access_token');
    });
  });
  ...
  describe('validateConfig', () => {
    it('should validate a valid configuration', () => {
      const config: ServerConfig = {
        ...
        airtable: {
          aIRTABLEACCESSTOKEN: 'test-token',
          api_version: 'v0'
        }
      };
    ...
    it('should require AIRTABLE_ACCESS_TOKEN', () => {
      const config: ServerConfig = {
        logShipping: {
          enabled: false,
          endpoint: '',
      ...
      expect(result.errors).toContain('AIRTABLE_ACCESS_TOKEN environment variable is required for airtable');
    });
